#ifndef ELL_PAGE_RANK_HPP
#define ELL_PAGE_RANK_HPP
...
class EllPageRank {
public:
    EllPageRank() : vectorInitialized_{false}, matrixInitialized_{false}, curXIdx_{0}, usedIterations_{0}, resultUpdated_{false}, precisionChanges_(NumberSegments+1, 0), precisionChanesTimePoint_(NumberSegments+1) {}
    ...
    bool initMatrix(const EllMatrix<IndexType, OriginalPointerType> &ellMtx);
    void resetVector();
...
private:
    ...
    IndexType m_, n_, k_;
    SplitType matVals_;
    std::vector<IndexType> matIndices_;
