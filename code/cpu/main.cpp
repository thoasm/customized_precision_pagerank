...
#include "ell_page_rank.hpp"
#include "csr_page_rank.hpp"
#include "ell_benchmark.hpp"
#include "csr_benchmark.hpp"
...
{
    /*
    double testing[] = {4, -23, -0, -0.0001};
    __m256d testingPack = _mm256_loadu_pd(testing);
    __m256d absTestingPack = avx2Helper::absPack(testingPack);
    double resultTest[4];
    _mm256_storeu_pd(resultTest, absTestingPack);
    
    for (int i = 0; i < 4; ++i) {
    std::cout << resultTest[i] << std::endl;
    }
    return 0;
    */
    /*
    constexpr std::size_t sizeVec = 101;
    SplitPointer<1, double> abc;
    newSplitPtr(&abc, sizeVec);
    for (std::size_t i = 0; i < sizeVec; ++i) {
        //abc.writeAll(i, static_cast<double>(i) / 2.0);
        abc.segmentPtr[i] = static_cast<double>(i) / 2.0;
    }
    double normRes = kernels::template norm1<1>(sizeVec, abc);
    
    std::cout << "Norm result: " << normRes << std::endl;
    ...
    */
    ...
    constexpr std::size_t numberKernelRunsToAverage = 50;
    constexpr std::size_t numberPageRankRunsToAverage = 5;
    ...
    */
    ...
    ssSettingsKernels << std::scientific << "% all kernel runtimes averaged over " << numberKernelRunsToAverage
            << " executions (and measured in ns); PageRank executed with damping factor of " << dampingFactor
                << "; Sparse format: CSR; Mode: interleaved with " << memoryBlockSize 
                << " Bytes; Using thread-buffer; ";
...
//*
    ...
    ssBasicBlocksValues << benchDouble.runRWTest<1>();
    ssBasicBlocksHeader << headlineSeparator << "SpMV " << pageRankVersionStr << " double";
    ssBasicBlocksValues << dataSeparator << benchDouble.runSpmv<1>();
    ssBasicBlocksHeader << headlineSeparator << "norm double";
    ssBasicBlocksValues << dataSeparator << benchDouble.runOwnNorm<1>();

    ssNormHeader << "% " << "double selective-norm";
    ssNormValues << benchDouble.runSelectiveNorm<1>();
    ssNormHeader << headlineSeparator << "double diff-norm";
    ssNormValues << dataSeparator << benchDouble.runDiffNorm<1>();
    ...
    auto bench2SegmentConversion = bench2Segment.runConversion();
    ...
    ssBasicBlocksValues << dataSeparator << bench2Segment.runRWTest<1>();
    ssBasicBlocksHeader << headlineSeparator << "read/write 2segment 64 bit";
    ssBasicBlocksValues << dataSeparator << bench2Segment.runRWTest<2>();

    ssBasicBlocksHeader << headlineSeparator << "SpMV " << pageRankVersionStr << " 2segment 32 bit";
    ssBasicBlocksValues << dataSeparator << bench2Segment.runSpmv<1>();
    ssBasicBlocksHeader << headlineSeparator << "SpMV " << pageRankVersionStr << " 2segment 64 bit";
    ssBasicBlocksValues << dataSeparator << bench2Segment.runSpmv<2>();

    ssBasicBlocksHeader << headlineSeparator << "norm 2segment 32 bit";
    ssBasicBlocksValues << dataSeparator << bench2Segment.runOwnNorm<1>();
    ssBasicBlocksHeader << headlineSeparator << "norm 2segment 64 bit";
    ssBasicBlocksValues << dataSeparator << bench2Segment.runOwnNorm<2>();

    ssNormHeader << headlineSeparator << "2segment 32 bit selective-norm";
    ssNormValues << dataSeparator << bench2Segment.runSelectiveNorm<1>();
    ssNormHeader << headlineSeparator << "2segment 64 bit selective-norm";
    ssNormValues << dataSeparator << bench2Segment.runSelectiveNorm<2>();

    ssNormHeader << headlineSeparator << "2segment 32 bit diff-norm";
    ssNormValues << dataSeparator << bench2Segment.runDiffNorm<1>();
    ssNormHeader << headlineSeparator << "2segment 64 bit diff-norm";
    ssNormValues << dataSeparator << bench2Segment.runDiffNorm<2>();
    ...
    auto bench4SegmentConversion = bench4Segment.runConversion();
...
/*/
    //TODO: remove!!!
    std::vector<std::uint64_t> bench2SegmentNormalizationRuntime(2, 1);
    std::vector<std::uint64_t> bench4SegmentNormalizationRuntime(4, 1);
    std::uint64_t bench2SegmentConversion = 1;
    std::uint64_t bench4SegmentConversion = 1;
//*/
    ...
    std::cout << "data_pagerank_speedups = [\n"
            << "% double" << headlineSeparator << "2 segment" << headlineSeparator << "4 segment\n"
            << static_cast<double>(pageDoubleRuntime[0]) / pageDoubleRuntime[0] << dataSeparator 
            << static_cast<double>(pageDoubleRuntime[0]) / page2SegmentRuntime[0] << dataSeparator
            << static_cast<double>(pageDoubleRuntime[0]) / page4SegmentRuntime[0] << dataSeparator
            << "\n];\n\n";
