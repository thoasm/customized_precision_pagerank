function [ data ] = plot_data_ell( save )
...
%eval('ell_juwels48_interleaved_181127_1223'); fileSuffix = '_cpu_interleaved_8KiB';
%eval('ell_juwels48_interleaved_128B_181128_0547'); fileSuffix = '_cpu_interleaved_128B';
eval('ell_v100_181128_0251'); fileSuffix = '_gpu_v100';
%eval('ell_juwels48_split_181128_0552'); fileSuffix = '_cpu_split';
...
ylim([.2 1.6]);
xlim([0 8]);
names = { 'Ada'; 'Del'; 'Eur'; 'Bub'; 'Rgg'; 'USA'; 'Std';  };
set(gca,'xtick',[1:1:7],'xticklabel',names); 
...
if( save == 1 )
    plotname = strcat('plots/rel_runtime_ell', fileSuffix);
...
set(gca,FS,fs),grid on
%names = { 'Adapt'; 'Delny'; 'Euro'; 'Bubble'; 'Rgg'; 'Rd\_USA';'Stanford';};
names = { 'Ada'; 'Del'; 'Eur'; 'Bub'; 'Rgg'; 'USA'; 'Std';  };
set(gca,'ytick',[2:5:35],'yticklabel',names')

%xlim([0 2.0]);

if( save == 1 )
    plotname = strcat('plots/trace_ell', fileSuffix);
    set(gcf, 'Color', 'white'); % white bckgr
    export_fig( gcf, ...       % figure handle
            plotname,...     % name of output file without extension
        '-painters', ...      % renderer
...
if( save == 1 )
    plotname = strcat('plots/rel_runtime_spmv_ell', fileSuffix);
    set(gcf, 'Color', 'white'); % white bckgr
    export_fig( gcf, ...       % figure handle
            plotname,...     % name of output file without extension
        '-painters', ...      % renderer
...
% The follwoing code is not needed for the CPU evaluation


eval('result_ell_v100_180810_0850');
...
if( save == 1 )
    plotname = strcat('plots/rel_runtime_ell_v100');
    set(gcf, 'Color', 'white'); % white bckgr
    export_fig( gcf, ...       % figure handle
            plotname,...     % name of output file without extension
        '-painters', ...      % renderer
...
xlabel('Normalized PageRank execution time',FS,fs);
set(gca,FS,fs),grid on
%names = { 'Stanford';'Adapt'; 'Delny'; 'Euro'; 'Bubble'; 'Rgg'; 'Rd\_USA';};
names = { 'Ada'; 'Del'; 'Eur'; 'Bub'; 'Rgg'; 'USA'; 'Std';  };
set(gca,'ytick',[2:5:35],'yticklabel',names')
...
if( save == 1 )
    plotname = strcat('plots/trace_ell_v100');
    set(gcf, 'Color', 'white'); % white bckgr
    export_fig( gcf, ...       % figure handle
            plotname,...     % name of output file without extension
        '-painters', ...      % renderer
...
if( save == 1 )
    plotname = strcat('plots/rel_runtime_spmv_ell_v100');
    set(gcf, 'Color', 'white'); % white bckgr
    export_fig( gcf, ...       % figure handle
            plotname,...     % name of output file without extension
        '-painters', ...      % renderer
